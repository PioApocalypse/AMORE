...
ssl_verification = True  # os.getenv('VERIFY_SSL').lower() == 'true' # this way you can toggle SSL verification in .env file
head = Header(api_key)
...
def caching():
    ...
    catfile = os.path.join(cache_dir, "categories.json")
    if os.path.isfile(catfile):
        with open(catfile, "r") as cf:
            cat = json.load(cf)
    elif os.path.isfile(
        "ok"
    ):  # either for debug or a really fucked up old attempt at error dodging
        pass
    else:
        response = requests.get(
            headers=head.dump(),
            url=os.path.join(full_elab_url, "items_types"),
            verify=ssl_verification,
        )
        cache.cache["categories"] = {
            cat.get("category_title").lower(): cat.get("id") for cat in response.json()
        }
        # Saving a better idea for later:
        # cache.cache["categories"] = {
        ...
        # }
        cache._save_to_disk("categories")
        # raise FileNotFoundError(
        #    f"No {filename} file found.\nPlease run amore/scan_elab.py."
        # )

    slotsfile = os.path.join(cache_dir, "slots.json")
    if os.path.isfile(slotsfile):
        with open(slotsfile, "r") as sf:
            slots = json.load(sf)
    else:
        # shortlist is a Tracker-class object
        cache.cache["slots"] = sample_locator(api_key).shortlist()
        cache._save_to_disk("slots")
    return
...
def sample_locator(API_KEY):
    ...
    header = Header(API_KEY).dump()
    response = requests.get(headers=header, url=search_query, verify=ssl_verification)
...
def move_sample(API_KEY, sample_id, new_position_name):
    ...
    header = Header(API_KEY).dump()
    sample = requests.get(
        headers=header, url=f"{items_url}/{sample_id}", verify=ssl_verification
    )
...
def add_to_position(
    API_KEY, sample_id, position_name, userid=""
):  # POST to empty position
    """
    ...
    patching_meta = requests.patch(
        headers=header, url=tracker_url, json=patch, verify=ssl_verification
    )
    link_url = f"{tracker_url}/items_links/{sample_id}"
    linking_item = requests.post(headers=header, url=link_url, verify=ssl_verification)
    return 0
...
def patch_tracker(API_KEY, sample_id, old_position_name, new_position_name, userid=""):
    ...
    """
    if old_position_name == new_position_name or (
        not old_position_name and not new_position_name
    ):  # neutral condition, just to be safe (both null or both equal)
    ...
    if (old_position_name == "") or (old_position_name is None):
        ...
        method = (
            "post"  # if old position doesn't exist, item is not linked yet so do it
        )
    ...
    try:
        ...
        linking_item = requests.request(
            method=method, headers=header, url=link_url, verify=ssl_verification
        )
    except:
        pass
    return
...
def move_to_position(
    API_KEY, sample_id, old_position_name, new_position_name, userid=""
):
    """
    ...
    """
    std_id = move_sample(
        API_KEY, sample_id, new_position_name
    )  # try to fetch the sample first
    patch_tracker(
        API_KEY, sample_id, old_position_name, new_position_name, userid
    )  # patch the sample locator experiment
    return std_id
...
def patch_sample(
    API_KEY, new_elabid, new_userid, body, std_id, position, batch, subholder, proposal
):
    """
    ...
    payload = {"custom_id": None, "body": body, "metadata": json.dumps(metadata)}
    response = requests.patch(
        url=itemurl, headers=header, json=payload, verify=ssl_verification
    )
    return 0
...
def upload_attachments(API_KEY, new_elabid, attachments):
    ...
    for field_name, (filename, file) in attachments:
        ...
        try:
            response = requests.post(
                url=uploads_url, headers=header, files=files, verify=ssl_verification
            )
...
def create_sample(
    ...
    payload = {
        "template": cat.get(
            "sample"
        ),  # 10 defines this item as 'sample' in our database
        "title": title,
        "tags": tags,
    }
    response = requests.post(
        url=items_url, headers=header, json=payload, verify=ssl_verification
    )
    ...
    add_to_position(
        API_KEY=API_KEY, sample_id=new_elabid, position_name=position, userid=new_userid
    )
    # upload attachments
    if attachments:
        upload_attachments(
            API_KEY=API_KEY, new_elabid=new_elabid, attachments=attachments
        )
...
def batch_pieces_reducer(API_KEY, batch):
    ...
    batch_data = requests.get(headers=header, url=batch_url, verify=ssl_verification)

    # Step 2: parse metadata from batch id
    batch_meta = json.loads(
        batch_data.json()["metadata"]
    ...
    payload_batch = {"metadata": json.dumps(batch_meta)}
    response = requests.patch(
        url=batch_url, headers=header, json=payload_batch, verify=ssl_verification
    )
...
def get_substrate_batches(API_KEY):
    ...
    """
    header = Header(API_KEY).dump()
    search_query = f"{items_url}?q=&cat={cat.get('substrate')}&limit=9999"  # "SUBSTRATES BATCH" should be id = 9
    response = requests.get(headers=header, url=search_query, verify=ssl_verification)
    ...
    batches = [
        ...
        for item in response.json()
        if to_int(
            json.loads(item["metadata"])["extra_fields"]["Available pieces"]["value"]
        )
        > 0  # which is an array of json objects/dictionaries
    ]
